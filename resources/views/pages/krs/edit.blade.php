            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="card-title mb-0 p-0 fs-6 fw-bold">
                        <i class="bi bi-pencil-square me-2 text-warning"></i>Form Edit Kartu Rencana Studi (KRS)
                    </h5>
                </div>
                    ...
                    <form action="{{ route('krs.update', $krs->id) }}" method="POST">
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-md-5 mb-3">
                                ...
                                <select name="mahasiswa_id" id="mahasiswa_id" class="form-select select2 @error('mahasiswa_id') is-invalid @enderror" data-placeholder="-- Pilih NIM / Nama Mahasiswa --" required>
                                    @foreach($mahasiswa as $mhs)
                                        <option value="{{ $mhs->id }}" {{ old('mahasiswa_id', $krs->mahasiswa_id) == $mhs->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="tahun_akademik" id="tahun_akademik" class="form-select @error('tahun_akademik') is-invalid @enderror" required>
                                    @foreach($tahunAkademik as $ta)
                                        <option value="{{ $ta->tahun_akademik }}" {{ old('tahun_akademik', $krs->tahun_akademik) == $ta->tahun_akademik ? 'selected' : '' }}>
                                            {{ $ta->tahun_akademik }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <select name="semester" id="semester" class="form-select @error('semester') is-invalid @enderror" required>
                                    @for($i = 1; $i <= 8; $i++)
                                        <option value="{{ $i }}" {{ old('semester', $krs->semester) == $i ? 'selected' : '' }}>Semester {{ $i }}</option>
                                    @endfor
                                </select>
                                @error('semester')
                                    <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="card-body p-3">
                                <div class="row">
                                    @forelse($mataKuliah as $mk)
                                        @php
                                            $checked = in_array($mk->id, old('mata_kuliah_id', $selectedMatkulIds));
                                        @endphp
                                        <div class="col-md-6 col-lg-4 mb-2">
                                            <div class="form-check border rounded p-2 ps-4 bg-white h-100">
                                                <input class="form-check-input" type="checkbox" name="mata_kuliah_id[]" value="{{ $mk->id }}" id="mk_{{ $mk->id }}" {{ $checked ? 'checked' : '' }}>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-warning text-white">
                                <i class="bi bi-check2-circle me-1"></i> Perbarui KRS
                            </button>
                        </div>
