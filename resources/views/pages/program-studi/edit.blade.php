            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="card-title mb-0 p-0 fs-6 fw-bold">
                        <i class="bi bi-pencil-square me-2 text-warning"></i>Form Edit Data Program Studi
                    </h5>
                </div>
                ...
                <div class="card-body pt-4">
                    <form action="{{ route('program-studi.update', $programStudi->id) }}" method="POST">
                            ...
                            <div class="col-sm-9">
                                <select name="fakultas_id" id="fakultas_id" class="form-select @error('fakultas_id') is-invalid @enderror" required>
                                    @foreach($fakultas as $item)
                                        <option value="{{ $item->id }}" {{ old('fakultas_id', $programStudi->fakultas_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->nama_fakultas }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-4">
                            ...
                            <div class="col-sm-9">
                                <input type="text" name="program_studi" id="program_studi" class="form-control @error('program_studi') is-invalid @enderror" value="{{ old('program_studi', $programStudi->program_studi) }}" placeholder="Contoh: S1-TEKNIK INFORMATIKA" required>
                                @error('program_studi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
