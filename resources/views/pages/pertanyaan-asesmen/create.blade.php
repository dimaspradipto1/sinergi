...
@section('content')
<div class="pagetitle">
    <h1>Tambah Pertanyaan Asesmen</h1>
    <nav>
        <ol class="breadcrumb">
            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="card-title mb-0 p-0 fs-6 fw-bold">
                        <i class="bi bi-question-circle me-2 text-primary"></i>Form Tambah Butir Pertanyaan Asesmen
                    </h5>
                </div>

                <div class="card-body pt-4">
                    <form action="{{ route('pertanyaan-asesmen.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="pertanyaan" id="pertanyaan" rows="4" class="form-control @error('pertanyaan') is-invalid @enderror" placeholder="Tuliskan teks pertanyaan atau butir pernyataan indikator asesmen..." required>{{ old('pertanyaan') }}</textarea>
                            @error('pertanyaan')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <select name="tipe_jawaban" id="tipe_jawaban" class="form-select @error('tipe_jawaban') is-invalid @enderror" required>
                                    <option value="" disabled {{ old('tipe_jawaban') ? '' : 'selected' }}>-- Pilih Tipe Jawaban --</option>
                                    @foreach($tipeJawabanList as $tipe)
                                        <option value="{{ $tipe }}" {{ old('tipe_jawaban') == $tipe ? 'selected' : '' }}>
                                            {{ $tipe }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="bobot" id="bobot" min="1" class="form-control @error('bobot') is-invalid @enderror" value="{{ old('bobot', 1) }}" placeholder="Contoh: 1, 5, 10" required>
...
B. Baik
C. Cukup
D. Kurang">{{ old('pilihan_jawaban') }}</textarea>
