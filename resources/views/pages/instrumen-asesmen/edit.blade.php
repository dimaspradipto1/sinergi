            ...
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="card-title mb-0 p-0 fs-6 fw-bold">
                        <i class="bi bi-pencil-square me-2 text-warning"></i>Form Edit Instrumen Asesmen
                    </h5>
                </div>

                <div class="card-body pt-4">
                    <form action="{{ route('instrumen-asesmen.update', $instrumen->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        <div class="mb-3">
                            <label for="nama_instrumen" class="form-label fw-semibold">Nama Instrumen Asesmen <span class="text-danger">*</span></label>
                            <input type="text" name="nama_instrumen" id="nama_instrumen" class="form-control @error('nama_instrumen') is-invalid @enderror" value="{{ old('nama_instrumen', $instrumen->nama_instrumen) }}" placeholder="Contoh: Soft Skill / Leadership / Digital Skill / Bahasa Inggris" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="kategori" id="kategori" class="form-select @error('kategori') is-invalid @enderror" required>
                                    @foreach($kategoriList as $kat)
                                        <option value="{{ $kat }}" {{ old('kategori', $instrumen->kategori) == $kat ? 'selected' : '' }}>
                                            {{ $kat }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="deskripsi" id="deskripsi" rows="3" class="form-control @error('deskripsi') is-invalid @enderror" placeholder="Penjelasan mengenai tujuan dan cakupan penilaian instrumen ini">{{ old('deskripsi', $instrumen->deskripsi) }}</textarea>
                            @error('deskripsi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="d-flex justify-content-end gap-2">
                            ...
                            <button type="submit" class="btn btn-warning text-white">
                                <i class="bi bi-check2-circle me-1"></i> Perbarui Instrumen
                            </button>
                        </div>
