...
class PortofolioDataTable extends DataTable
{
     ...
     *
     * @param QueryBuilder<Portofolio> $query Results from query() method.
     */
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('kategori_badge', function ($row) {
                return '<span class="badge bg-primary">' . e($row->kategori) . '</span>';
            })
            ->addColumn('file_preview', function ($row) {
                if ($row->file) {
                    $url = asset('storage/' . $row->file);
                    return '<a href="' . $url . '" target="_blank" class="btn btn-sm btn-outline-primary" title="Lihat Berkas Portofolio">
            ...
            ->addColumn('action', function ($row) {
                $editUrl = route('portofolio.edit', $row->id);
                $deleteUrl = route('portofolio.destroy', $row->id);
                $csrf = csrf_field();
                $methodDelete = method_field('DELETE');

                return '
                <div class="d-flex justify-content-center align-items-center gap-1">
                    <a href="' . $editUrl . '" class="btn btn-sm btn-warning text-white" title="Edit Portofolio">
                        <i class="bi bi-pencil-square"></i>
                    </a>
                    <form action="' . $deleteUrl . '" method="POST" class="d-inline form-delete">
                        ' . $csrf . '
                        ' . $methodDelete . '
                        <button type="button" class="btn btn-sm btn-danger btn-delete" data-name="' . e($row->judul) . '" title="Hapus Portofolio">
                            <i class="bi bi-trash"></i>
                        </button>
                    </form>
                </div>';
            })
            ->rawColumns(['kategori_badge', 'file_preview', 'action'])
            ->setRowId('id');
    }
     ...
     *
     * @return QueryBuilder<Portofolio>
     */
    public function query(Portofolio $model): QueryBuilder
    {
        return $model->newQuery()->with('mahasiswa')->select('portofolios.*');
    }
        ...
        return $this->builder()
                    ...
                    ->parameters([
                        ...
                        'language' => [
                            ...
                            'zeroRecords' => 'Data portofolio tidak ditemukan',
                            'paginate' => [
                                'first' => 'Awal',
                                'last' => 'Akhir',
                                'next' => 'Berikutnya',
